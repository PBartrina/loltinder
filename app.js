...
console.log("** CoMe oN LiKe MaChInE!**");
...
bot.client.authorize(bot.access_token, bot.facebook_id, function() {
    bot.client.updatePosition("41.4239352", "2.1660459", function() {
        ...
        interval = setInterval(function() {
            ...
            bot.client.getRecommendations(10, function(error, data) {
                ...
                _.chain(data.results)
                    .each(function(v, k) {
                        var friends = (v.common_friend_count) ? v.common_friend_count : "0";
                        var bio = v.bio.toLowerCase();

                        if (s.include(bio, "out of likes")) {
                            console.log("tas pasao de likes, tete");
                            clearInterval(interval);
                        } else if (v.common_friend_count || s.include(bio, "trans")) {
                            bot.client.pass(v._id, function(error, data) {
                                if (s.include(bio, "trans")) console.log(v.name + " it's a TRAP!");
                                if (v.common_friend_count) {
                                    console.log(v.name, "is friend of: ", v.common_friends);
                                }
                            });
                        } else {
